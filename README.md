...
Для поддержания качества и стиля кода рекомендуется использовать:
...
Установка и запуск:
1. Добавьте инструменты в environment.yml:
   ```yaml
   - pip:
       - ruff
       - black
       - mypy
   ```
2. Установите зависимости: `conda env update -f environment.yml`
3. Запуск линтера: `ruff src/`
4. Запуск автоформаттера: `black src/`
5. Запуск проверки типов: `mypy src/`
